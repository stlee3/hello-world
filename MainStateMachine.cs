...
namespace CCEApplication.StateMachine
{
    public class MainStateMachine
    {
        ...
        public MainStateMachine(Machine pStationOwner, DigitalIOInterface IOInterface)
        {
       ...
       ~MainStateMachine()
        ...
        private void DoSequence(enumActivity eSequence)
        {
            switch (eSequence)
            {
                ...
                case enumActivity.eCompleted:
                    ...
                    break;
                default:
                    MessageBox.Show("Main State Machine DoSequence ERROR. Please restart program.");
                    break;
            }
        }
            ...
            switch (eStateNo)
            {
               case enumActivity.eProduction:
                ...
                case enumActivity.eStopping:
                    ...
                    break;
                default:
                    MessageBox.Show("Main State Machine BranchToStateProcess ERROR. Please restart program.");
                    break;
            }
        }
        ...
        private void OnExecuteIsCassettePresent()
        {
            ...
            if(GlobalVariable.bFrontRFID_Line1 == true)
             {
                 if(m_pStation.IsFrontRFIDAtLane1CassettePresentSensorActive())
                 {
                     DoSetNextSequence();
                    ReturnFromState();
                 }
             }
             else if (GlobalVariable.bRearRFID_Line1== true)
             {
                 if (m_pStation.IsRearRFIDAtLane1CassettePresentSensorActive())
                 {
                     DoSetNextSequence();
                     ReturnFromState();
                 }
        ...
        private void OnExecuteReadRF()
        {
            ...
            if (GlobalVariable.CCE != null )
                GlobalVariable.CCE.ReadRFDetailLane1(nID);
            Thread.Sleep(100);
            if (GlobalVariable.CCE != null )
                GlobalVariable.CCE.DisplayResultLane1();

            if (GlobalVariable.bDisplayLine1 || CountDetail >= 3)
            {
                DoSetNextSequence();
                ReturnFromState();
                CountDetail = 0;
            ...
            {
                if(GlobalVariable.CCE != null)
                GlobalVariable.CCE.ClearDisplay_1();
                CountDetail++;
            }
        }
